...
<main><section class="hero" aria-label="当前章节"><img src="{{IMAGE}}" alt="{{SCENE}}的氛围插画，物品归属以行囊记录为准"><div class="hero-text"><div class="eyebrow">{{PHASE}}</div><h1>{{TITLE}}</h1><p>{{SUBTITLE}}</p><small>在 WorkBuddy 对话里行动；这里替你记住已经发生的事。</small></div></section>
...
<div class="help">在 WorkBuddy 对话中，用自己的话描述行动即可。</div><p class="muted"><small>插画只表现氛围；场景、物品与关系以本页文字记录为准。面板是本地快照；操作后刷新查看。分享游戏包不会分享你的存档。</small></p>
